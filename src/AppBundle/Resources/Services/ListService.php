...
class ListService{
  ...
  public function liaisonSubjectsList(array $entities){    
    ...
    foreach($entities as $entity){
      ...
      if($level == 0){
        $styled_list .= 
            '<a href="#item-'.$parent_count.'" class="list-group-item" data-toggle="collapse">'.$entity->getName().'<span class="badge" onclick="location.href=\''.$this->router->generate('liaisonsubject_edit', array('id'=>$entity->getId())).'\'">Edit</span></a>';
        $styled_list .= 'Primary Liaison';
        $styled_list .= '<div class="list-group" id="item-'.$parent_count.'">';
        ...
        foreach($children as $child){
          $styled_list .= 
              //'<a href="#item-'.$parent_count.'-'.$child_count.'" class="list-group-item" data-toggle="collapse"><i class="glyphicon glyphicon-chevron-right"></i>'.$child->getTitle().'</a>';
              '<a href="#item-'.$parent_count.'-'.$child_count.'" class="list-group-item" data-toggle="collapse">'.$child->getName().'<span class="badge" onclick="location.href=\''.$this->router->generate('liaisonsubject_edit', array('id'=>$child->getId())).'\'">Edit</span></a>';
          $styled_list .= '<div class="list-group collapse in" id="item-'.$parent_count.'-'.$child_count.'">';
          ...
          foreach($grandchildren as $grandchild){
            $styled_list .= 
              '<a href="#" class="list-group-item">'.$grandchild->getName().'<span class="badge" onclick="location.href=\''.$this->router->generate('liaisonsubject_edit', array('id'=>$grandchild->getId())).'\'">Edit</span></a>';
            $styled_list .= '<div class="list-group collapse in" id="item-'.$parent_count.'-'.$child_count.'-'.$grandchild_count.'">';
            $styled_list .= '</div>';
            $grandchild_count++;
          }
    ...
    foreach($entities as $entity){
      ...
      if($level == 0){
        ...
        foreach($children as $child){
          ...
          $styled_list .= '<td>Add Phone number field!</td>';
          $styled_list .= '</tr>';
